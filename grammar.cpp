...
namespace compiler {
	...
	void Grammar::readstatement(int level) {
		...
		if (lex.sym == kLBracket) {
			lex.getsym();
			if (lex.sym == kIdent) {
				emit("RED", level - tables[position(lex.last_id, 0, 0)].level, tables[position(lex.last_id, 0, 0)].address);
				lex.getsym();
				while (lex.sym == kComma) {
					lex.getsym();
					if (lex.sym != kIdent) {
	...
	void Grammar::enter(int level, std::string name, SymType kind, int size, int number) {
	...
	void Grammar::enter(int level, std::string name, SymType kind, int size, double real) {
	...
	void Grammar::enter(int level, std::string name, SymType kind, int size, char c) {
	...
	void Grammar::printTable() {
		...
		for (auto table : tables) {
			std::cout << "name:"<<table.name << "   kind:" << table.kind << "   level:" <<table.level<<"    addr:"<< table.address << std::endl;
		}
	}
	...
	void Grammar::printCode() {
		for (auto code : codes) {
			...
			//}
			//else if (code.op <= 30) {
			//	std::cout << CodeSymbol[code.op] << " " << code.x << " " << code.y << std::endl;
			//}
			//else {
